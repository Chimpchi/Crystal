...
void PreferencesWindow::OnWindowAdded(void)
{
    Preferences &preferences = m_application->GetPreferences();
    Preferences::GlobalSettings &settings = preferences.GetGlobalSettings();
    m_selectedFont = settings.GetFontIndex(settings.GetCurrentFontName());
    m_selectedTheme = settings.GetColorThemeIndex(settings.GetCurrentColorThemeName());
}
...
void PreferencesWindow::RenderWindow(void)
{
    ...
    if (ImGui::Begin("Preferences##Preferences", &m_opened, 0))
    {
        ...
        ImGui::TextColored(headerColor, "Global Settings");
        {
            ...
            settings.fontSize = Clamp(settings.fontSize, 13.0f, 64.0f);
            ImGui::TextUnformatted("Editor Font Size");
            ImGui::DragFloat("##EditorFontSize", &settings.editorFontSize, 0.1f, 13.0f, 64.0f, "%.1f");
            settings.editorFontSize = Clamp(settings.editorFontSize, 13.0f, 64.0f);
